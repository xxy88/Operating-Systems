...
__device__ __managed__ uchar storage_bitmap[(PHYSICAL_MEM_SIZE + STORAGE_SIZE) / PAGE_SIZE];
...
__global__ void mykernel(int input_size) {
    ...
    vm_init(&vm, data, storage, pt, swap_table, storage_bitmap, swap_buffer, &pagefault_num, PAGE_SIZE,
            INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE, PHYSICAL_MEM_SIZE / PAGE_SIZE);
