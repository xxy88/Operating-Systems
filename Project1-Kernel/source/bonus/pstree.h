...
// create a proc_node and append it into the global node array.
...
char tree_equal(const proc_node *a, const proc_node *b);

// trim a tree's duplicate node
void trim_tree(proc_node *root);

// recursively print the process tree.
void print_tree(proc_node *root, const char *indent, int count);
